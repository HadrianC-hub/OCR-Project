"""
Pipeline especializado para preprocesar imagenes que ya son una sola linea
manuscrita recortada (caso tipico de un dataset de OCR linea-a-linea).

Diferencias respecto a pipeline.run:
  ...
  - No usa mask_binding_strips (esa funcion trata columnas densas como
    encuadernacion y se come letras altas como Y, S, J al principio o final
    de una linea recortada).
  ...
  - La limpieza de ruido respeta cualquier componente conexo que toque el
    ...
    en el recorte previo de la linea).

Pasos:
  1. Conversion a grayscale eligiendo el canal con mas contraste tinta/papel.
  2. Denoising bilateral suave para limpiar textura del papel sin difuminar trazos.
  3. Normalizacion de iluminacion (compensa sombras y gradientes del escaner).
  4. Estimacion de inclinacion a partir de centroides Y por rebanada vertical
     y rotacion de la imagen grayscale con interpolacion bicubica.
  5. Binarizacion Sauvola con k bajo (0.12) para conservar trazos finos.
  ...
  8. Recorte vertical (solo top/bottom) con margen para ascenders/descenders.
  9. Redimensionado a altura objetivo manteniendo aspect ratio.
...
@dataclass
class LineConfig:
    # Configuracion del preprocesador de lineas. Defaults para handwriting
    # tipico (boli o lapiz sobre papel blanco, ~60-100 px de altura de texto).

    # Salida
    target_height:     int   = 64     # altura final fija (divisible entre 16)
    min_output_width:  int   = 32     # ancho minimo para evitar tensores degenerados
    ...
    normalize_bg:      bool  = True   # corregir iluminacion no uniforme

    # Deskew
    deskew:            bool  = True
    max_skew_angle:    float = 8.0
    straighten:        bool  = True   # correccion polinomial post-binarizacion

    # Binarizacion (Sauvola)
    ...
    noise_max_area:    int   = 8        # componentes <= N px^2 son ruido
    preserve_edges:    bool  = True     # no eliminar componentes pegados al borde lateral
    remove_rule_lines: bool  = True     # eliminar rayas horizontales de cuaderno
    rule_line_max_h:    int   = 3       # altura max en px de un trozo de pauta
    rule_min_coverage:  float = 0.20    # longitud minima de raya = fraccion x ancho
    rule_max_angle_deg: float = 4.0     # tolerancia angular para considerar pauta

    # Recorte vertical (no horizontal)
...
class LineResult:
    ...
    #   image: imagen final uint8 (fondo blanco, tinta negra) redimensionada
    #     a cfg.target_height. Lista para entrenar OCR.
    #   binary_full: binaria a resolucion completa post-rotacion, antes del
    #     resize final. Util para inspeccion o entrenamiento con resolucion
    #     variable.
    #   angle_deg: angulo de deskew aplicado (grados, positivo CCW).
    #   text_height_est: altura estimada del texto en px (post-rotacion).
    #   warnings: avisos no fatales (binarizacion dudosa, etc).
    image:           np.ndarray
...
def _to_gray_best(img: np.ndarray, pick_best: bool) -> np.ndarray:
    # Convierte a grayscale eligiendo, si procede, el canal con mas rango
    # dinamico (p95 - p5). Para tinta azul, el canal R suele ofrecer mas
    # contraste que el gris promedio. Para tinta negra los tres canales dan
    # resultados similares y se queda con gray.
    if img.ndim == 2:
        return img.copy()
...
def _deskew_grayscale(
    ...
    # Estima inclinacion con una binarizacion rapida y ajuste de baseline
    # (rotate_strip_by_baseline), y aplica la rotacion inversa sobre la imagen
    # GRAYSCALE con interpolacion cubica. Rotar grayscale antes de Sauvola
    # produce bordes mas limpios que rotar la binaria. El borde rotado se
    # rellena con la mediana de las 4 aristas para no introducir bandas oscuras.
    if gray.size == 0:
        return gray, 0.0
...
def _binarize_with_retry(
    ...
    # Sauvola con auto-ajuste de k. Si la fraccion de tinta queda fuera del
    # rango deseado, baja k (mas tinta) o lo sube (menos tinta) hasta entrar
    # o agotar iteraciones. Tracking del mejor candidato: si ninguna iteracion
    # cae en el rango, se devuelve la mas cercana al centro.
    lo, hi = target_ink
    center = 0.5 * (lo + hi)
...
def _remove_horizontal_rule_lines(
    ...
    # Elimina rayas de pauta de cuaderno (rule lines), tolerando inclinacion.
    #
    # Las pautas son trazos horizontales muy delgados (1-3 px) que recorren
    # la pagina. Cuando el texto cruza la pauta, queda partida en muchos
    # fragmentos cortos. Los fragmentos pueden estar desalineados en Y porque
    # la pauta no es perfectamente horizontal y porque el deskew se ajusta al
    # baseline del texto, no a la pauta.
    #
    # Estrategia:
    #   1. Aislar componentes delgados (altura <= max_h). Letras, acentos,
    #      puntos sobre i (h >= 4-5 px) quedan protegidos desde el inicio.
    #   2. Clausura morfologica horizontal suave para unir fragmentos vecinos.
    #   3. Deteccion Hough de segmentos casi horizontales: una raya verdadera
    #      genera lineas con |angulo| <= max_angle_deg y longitud >= 
    #      min_coverage_frac * W.
    #   4. Construir una banda alrededor de cada raya y eliminar todos los
    #      componentes delgados que la atraviesen.
    H, W = binary.shape
    ...
    # Kernel angosto (W/40) para puentear pequenos huecos, sin unir cosas
    # no colineales.
    k_w = max(8, W // 40)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (k_w, 1))
    closed = cv2.morphologyEx(thin_mask, cv2.MORPH_CLOSE, kernel)

    # Hough probabilistico: segmentos largos casi horizontales.
    # minLineLength exige longitud >= min_coverage_frac * W para filtrar letras.
    ...
    segments = cv2.HoughLinesP(
        closed,
        rho           = 1,
        theta         = np.pi / 720,    # resolucion 0.25 grados
        threshold     = hough_thr,
    ...
    # Filtrar por angulo y extrapolar cada segmento al ancho completo:
    # una pauta casi siempre recorre la pagina entera, y Hough puede recortar
    # los extremos si los fragmentos finales son escasos.
    rule_segments = []
    for seg in segments:
        x1, y1, x2, y2 = seg[0]
        dx = x2 - x1
    ...
    # Banda de eliminacion: corredor vertical alrededor de cada raya. Como la
    # pauta puede tener ligera curvatura no capturada por el modelo lineal,
    # ensanchamos a (2*max_h + 3) px. Componentes altos (letras, acentos)
    # siguen protegidos al no estar en thin_ids.
    band = np.zeros((H, W), dtype=np.uint8)
    band_thickness = max(5, 2 * max_h + 3)
    for x1, y1, x2, y2 in rule_segments:
        cv2.line(band, (x1, y1), (x2, y2), 1, thickness=band_thickness)

    # Eliminar componentes delgados cuyo centroide cae dentro de la banda, o
    # que tengan >= 50% de sus pixeles dentro de ella.
    out = binary.copy()
    for lid in thin_ids:
        x, y, w, h, _ = stats[lid]
        cy = y + h // 2
...
def _clean_speckles(
    ...
    # Elimina componentes conexos con area <= max_area (ruido de granulado,
    # motas de polvo, artefactos JPEG).
    #
    # Conserva:
    #   - Cualquier componente con area > max_area.
    #   - Si preserve_edges: cualquier componente que toque el borde izquierdo
    #     o derecho. Razon: el recorte previo de la linea pudo cortar una
    #     letra justo en el borde y dejar un fragmento pequeno que SI es texto.
    #
    # No filtra por bordes superior/inferior porque ahi los huerfanos sí
    # suelen ser ruido (proyeccion de lineas vecinas).
    H, W = binary.shape
...
def _vertical_trim_only(binary: np.ndarray, margin: int) -> np.ndarray:
    # Recorta solo arriba y abajo a las filas con tinta + margen. El ancho NUNCA se toca.
    H, W = binary.shape
...
def _resize_to_height(img: np.ndarray, target_h: int, min_w: int) -> np.ndarray:
    # Redimensiona conservando aspect ratio.
    # Reduccion -> INTER_AREA (antialiasing). Ampliacion -> INTER_CUBIC (mas suave).
    H, W = img.shape[:2]
    if H == 0 or W == 0:
...
def preprocess_line(
    ...
    # Procesa una imagen que ya es una sola linea manuscrita recortada.
    # Acepta numpy array (BGR o grayscale) o ruta a archivo. Devuelve un
    # LineResult con la imagen lista para OCR y la binaria a resolucion plena.
    # El pipeline preserva el ancho original en TODAS las etapas; solo recorta
    # arriba y abajo.
    if cfg is None:
        cfg = LineConfig()
    ...
    if img is None or img.size == 0:
        raise ValueError("Imagen vacia o no cargable")
    ...
    # 3. Normalizacion de iluminacion (compensa sombras del escaner)
    if cfg.normalize_bg:
        gray = normalize_illumination(gray)

    # 4. Deskew sobre grayscale (rotacion con bicubic, sin aliasing)
    angle = 0.0
    if cfg.deskew:
        gray, angle = _deskew_grayscale(gray, max_angle=cfg.max_skew_angle)
        if cfg.debug and abs(angle) > 0.1:
            print(f"  [line_preprocess] deskew={angle:.2f} grados")
    ...
    # 6. Binarizacion con auto-ajuste de k
    ...
    if cfg.debug:
        ink_ratio = float((binary < 128).mean())
        print(f"  [line_preprocess] text_h~{text_h}px win={win} "
              f"k={k_used:.3f} ink={ink_ratio:.3f}")
    ...
    # 10. Recorte vertical unicamente
    binary_trim = binary
    if cfg.vertical_trim:
        binary_trim = _vertical_trim_only(binary, margin=cfg.vertical_margin)

    # Si el recorte vertical dejo el bbox vacio (no habia tinta), aviso y se
    # devuelve un lienzo blanco con tamano objetivo para no romper el batch.
    if (binary_trim < 128).sum() == 0:
        warns.append("No se detecto tinta tras la binarizacion; "
                     "se devuelve un lienzo en blanco.")
    ...
    out = _resize_to_height(binary_trim, target_h=cfg.target_height,
                            min_w=cfg.min_output_width)
