"""
ocr_engine.py — Motor OCR del proyecto.

Carga los modelos CRNN+CTC de forma diferida (lazy, thread-safe) y expone
las funciones ocr_printed() y ocr_manuscript() para usarse desde views.py.

──────────────────────────────────────────────────────────────────────────
FLUJO POR IMAGEN COMPLETA — IMPRESO
──────────────────────────────────────────────────────────────────────────
    1. preprocessing.pipeline.run()
       → detecta bloques + líneas + binariza + recorta cada línea
    2. OCRPredictor.predict() sobre cada line_crop
    3. join con '\\n' → texto completo de la página

──────────────────────────────────────────────────────────────────────────
FLUJO POR IMAGEN COMPLETA — MANUSCRITO   (cambiado 2026-05-12)
──────────────────────────────────────────────────────────────────────────
    1. preprocessing.manuscript_lines.detect_manuscript_lines()
       → detector robusto para cursiva (no recorta laterales, alto
         min_line_height, fusión cuerpo+descendentes vía closing
         vertical, split recursivo de bandas oversized)
    2. Para cada línea detectada: cropear bbox del binario deskewed,
       pasarlo por preprocessing.line_preprocess.preprocess_line()
       (mismo pipeline que se usó para preparar los datos de training)
    3. HTRPredictor.predict() sobre el array resultante
    4. join con '\\n' → texto completo de la página

¿Por qué un flujo distinto para manuscritos? Tres bugs documentados de
`pipeline.run` sobre cursiva:
  · Descendentes largos detectados como segunda línea → modelo recibe
    half-crops cortados a mitad de letra → predicción corrompida.
  · Espacios entre palabras detectados como columnas → cada línea se
    parte por la mitad y se devuelven en orden incorrecto.
  · `mask_binding_strips` puede recortar letras altas (Y, S, J, etc.)
    cerca de los bordes — el modelo entrenó con imágenes que SÍ
    conservan esos bordes (line_preprocess preserva componentes que
    tocan los bordes laterales).
Ver `preprocessing/manuscript_lines.py` para detalle.

──────────────────────────────────────────────────────────────────────────
FLUJO POR REGIONES DEFINIDAS POR EL USUARIO
──────────────────────────────────────────────────────────────────────────
    ocr_regions(image_path, regions, doc_type) — recorta cada región
    del facsimilar original y pasa cada recorte por el pipeline+OCR
    correspondiente al tipo de documento.

──────────────────────────────────────────────────────────────────────────
THREAD-SAFETY
──────────────────────────────────────────────────────────────────────────
Cada singleton de modelo está protegido por un Lock. La carga inicial es
costosa (HTRPredictor: ~5 s + ~120 MB; OCRPredictor: ~3 s + ~150 MB;
kenLM: ~30 s + ~500 MB). Si dos threads piden el mismo modelo a la vez
sin lock, ambos lo cargan en paralelo: doble RAM y doble tiempo. Con el
lock, el segundo espera al primero y reutiliza la instancia.

Además, limitamos los threads internos de PyTorch a 2 (en lugar de "todos
los cores"). El motivo: el OCR background thread es CPU-bound; si se
queda con todos los cores, las peticiones HTTP del usuario (incluyendo
la submit del siguiente documento) compiten por GIL+CPU y la web parece
colgada. Con torch limitado a 2 threads, Django siempre tiene cores
libres para responder. Override con la env var `OCR_TORCH_THREADS`.
"""
